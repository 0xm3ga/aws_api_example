...
import pytest
from api.v1.src.lambdas.goodbye_lambda.app import (
    InvalidNameError,
    generate_response,
    greet_user,
    handle_exception,
...
def test_greet_user_when_given_valid_name_returns_expected_message():
    ...
    assert greet_user("Test") == {"message": "Goodbye, Test!"}
...
def test_generate_response_when_given_valid_body_returns_expected_response():
    ...
    response = generate_response({"message": "Goodbye, Test!"})
    ...
    assert response["body"] == '{"message": "Goodbye, Test!"}'


@pytest.mark.unit
@pytest.mark.usefixtures("mock_env")
@patch(
    "api.v1.src.lambdas.goodbye_lambda.app.validate_input", return_value="Test"
)
@patch(
    "api.v1.src.lambdas.goodbye_lambda.app.greet_user",
    return_value={"message": "Goodbye, Test!"},
)
@patch(
    "api.v1.src.lambdas.goodbye_lambda.app.generate_response",
...
@pytest.mark.usefixtures("mock_env")
@patch(
    "api.v1.src.lambdas.goodbye_lambda.app.validate_input",
    side_effect=InvalidNameError("Invalid name provided"),
)
@patch(
    "api.v1.src.lambdas.goodbye_lambda.app.handle_exception",
